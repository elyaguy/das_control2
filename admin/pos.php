...
<head>
	...
	<style type="text/css">
		body::after {
			content: "";
			background: url(../assets/das/img/pos/patterns/<?php echo $user->getPreference('pos_pattern') ? $user->getPreference('pos_pattern') : 'armysuit.jpg'; ?>) repeat repeat;
			opacity: 0.4;
...
<body id="pos-page" class="pos sidebar-mini <?php echo $body_class; ?>" ng-controller="PosController">
				...
				<div class="content-row">
					...
					<div id="left-panel" class="pos-content" style="<?php echo $user->getPreference('pos_side_panel') == 'left' ? 'float:right' : null; ?>">
						<div class="contents">
							...
							<div id="searchbox" ng-hide="hideProductDropdown">
								...
								<div class="course-search">
									<select class="form-control select2" name="course-search-select" id="course-search-select">

										<option value=""><?php echo sprintf(trans('text_view_all'), 'Colleges');  ?></option>
										<?php foreach (get_courses() as $course) : ?>
											<option value="<?php echo $course['course_id']; ?>">
												<?php echo $course['course_name']; ?>
											</option>
										<?php endforeach; ?>
										<!-- <?/*php foreach (get_courses(array('filter_fetch_all' => true)) as $course_id => $course_name) :
										*/?>
											<option value="<?/*php echo $course_id; */?>"><?/*php echo $course_name;  */?> (<?/* echo get_total_valid_course_item($course_id); */?>)</option>
										<?/*php endforeach;  */?> -->
									</select>
								</div>
					...
					<div id="right-panel" class="pos-content" style="<?php echo $user->getPreference('pos_side_panel') == 'left' ? 'float:left' : null; ?>">
						<div class="invoice-area">
							<div class="well well-sm">
								...
								<!-- Customer Area Start-->
								<div id="college-area">
									<input ng-change="showCollegeList()" onClick="this.select();" type="text" id="college-name" name="college-name" ng-model="collegeName" ng-disabled="isEditMode" autocomplete="off">
									<input type="hidden" name="college-id" value="{{ collegeId }}">
									<div class="customer-icon">
										<a ng-click="showCollegeList(true)" onClick="return false;" href="#">
											<svg class="svg-icon">
												<use href="#icon-pos-customer">
											</svg>
										</a>
	...
	<div ng-click="createNewCollege();" class="add-icon hidden">
		<svg class="svg-icon">
			<use href="#icon-pos-plus">
		</svg>
	</div> -->
				...
				<div ng-show="!dueAmount">
					<span id="dueAmount">
						{{ dueAmount| formatDecimal:2 }}
									...
									<div ng-hide="hideCollegeDropdown" id="college-dropdown" class="slidedown-menu">
										<div class="slidedown-header">
										</div>
										<div class="slidedown-body">
											<ul class="college-list list-unstyled">
												<li ng-repeat="colleges in collegeArray">
													<a href="#" ng-click="addCollege(colleges);" onclick="return false;"><span class="fa fa-fw fa-home"></span>{{ colleges.college_name }} ({{ colleges.college_name || colleges.college_name }})
													</a>
												</li>
											</ul>
										</div>
										...
										<table class="table">
											<tbody>
												...
												<tr class="pay-top">
													...
													<td class="text-right">
														<input id="discount-input" ng-change="addDiscount()" onClick="this.select();" type="text" name="discount-amount" ng-model="discountInput" ondrop="return false;" onpaste="return false;" autocomplete="off" ng-disabled="!applyDiscount" >
													</td>
													<td>
