window.angularApp.controller("ReportStockController", [
    ...
    function (
        ...
        var i;
        var printColumns = stockReportDT.data("print-columns");
        var okFix = printColumns.split(",")[0];
        printColumns = printColumns.replace("-1,","");
                ...
                "drawCallback": function (settings) {
                    ...
                    api.column(1, { page: 'current' }).data().each(function (group, i) {
                        ...
                        if (typeof available[availabilityRow] != 'undefined') {
                            available[availabilityRow] = available[availabilityRow] + intVal(api.column(4).data()[i]);
                        } else {
                            available[availabilityRow] = intVal(api.column(4).data()[i]);
                        }

                        var totalSellPriceRow = generateHashCode(group) + '_sellamount';
                        if (typeof total[totalSellPriceRow] != 'undefined') {
                            total[totalSellPriceRow] = total[totalSellPriceRow] + intVal(api.column(5).data()[i]);
                        } else {
                            total[totalSellPriceRow] = intVal(api.column(5).data()[i]);
                        }

                        var totalpurchasePriceRow = generateHashCode(group) + '_purchaseamount';
                        if (typeof total[totalpurchasePriceRow] != 'undefined') {
                            total[totalpurchasePriceRow] = total[totalpurchasePriceRow] + intVal(api.column(6).data()[i]);
                        } else {
                            total[totalpurchasePriceRow] = intVal(api.column(6).data()[i]);
                        }

                        if (last !== group) {
                            if (okFix == "-1") {
                                $(rows).eq(i).before(
                                    '<tr class="group warning" data-child="' + generateHashCode(group) + '"><td class="bg-green" colspan="2" data-title="Supplier Name"><i class="fa fa-fw fa-angle-right"></i> ' + group + '</td><td class="bg-green text-right" data-title=""></td><td class="bg-green ' + availabilityRow + '  text-right" data-title="Total Sell Price"></td></tr>'
                                );
                            } else {
                                $(rows).eq(i).before(
                                    '<tr class="group warning" data-child="' + generateHashCode(group) + '"><td class="bg-green" colspan="2" data-title="Supplier Name"><i class="fa fa-fw fa-angle-right"></i> ' + group + '</td><td class="bg-green text-right" data-title=""></td><td class="bg-green ' + availabilityRow + '  text-right" data-title="Total Available"></td><td class="bg-green ' + totalSellPriceRow + '  text-right" data-title="Total Sell Price"></td><td class="bg-green ' + totalpurchasePriceRow + '  text-right" data-title="Total purchase Price"></td></tr>'
                                );
                            }
                            last = group;
                        }
