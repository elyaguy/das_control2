window.angularApp.controller("CourseController", [
    "$scope",
    "API_URL",
    "window",
    "jQuery",
...
function (
    ...
    dt.dataTable({
        ...
        "columnDefs": [
            ...
            { 
                ...
                'createdCell':  function (td, cellData, rowData, row, col) {
                   $(td).attr('data-title', $("#course-course-list thead tr th:eq(0)").html());
                }
            },
            { 
                "targets": [1],
                'createdCell':  function (td, cellData, rowData, row, col) {
                   $(td).attr('data-title', $("#course-course-list thead tr th:eq(1)").html());
                }
            },
            { 
                "targets": [2],
                'createdCell':  function (td, cellData, rowData, row, col) {
                   $(td).attr('data-title', $("#course-course-list thead tr th:eq(2)").html());
                }
            },
            { 
                "targets": [3],
                'createdCell':  function (td, cellData, rowData, row, col) {
                   $(td).attr('data-title', $("#course-course-list thead tr th:eq(3)").html());
                }
            },
            { 
                "targets": [4],
                'createdCell':  function (td, cellData, rowData, row, col) {
                   $(td).attr('data-title', $("#course-course-list thead tr th:eq(4)").html());
                }
            },
            { 
                "targets": [5],
                'createdCell':  function (td, cellData, rowData, row, col) {
                   $(td).attr('data-title', $("#course-course-list thead tr th:eq(5)").html());
                }
            },
            { 
                "targets": [6],
                'createdCell':  function (td, cellData, rowData, row, col) {
                   $(td).attr('data-title', $("#course-course-list thead tr th:eq(6)").html());
                }
            },
        ],
        "aoColumns": [
        ...
        "buttons": [
            ...
            },
            {
                ...
                titleAttr: "Copy",
                title: window.store.name + " > Course List",
                exportOptions: {
                    columns: [ 0, 1, 2, 3 ]
                }
            },
    ...
    // Edit course
    $(document).delegate("#edit-course", "click", function(e) {
        e.stopPropagation();
        e.preventDefault();
    ...
    // Delete course
    $(document).delegate("#delete-course", "click", function(e) {
        e.stopPropagation();
        e.preventDefault();
        var datatable = dt;
    ...
    if (window.getParameterByName("course_id") && window.getParameterByName("course_name")) {
        supId = window.getParameterByName("course_id");
        var supName = window.getParameterByName("course_name");
        dt.DataTable().search(supName).draw();
        dt.DataTable().ajax.reload(function(json) {
            $.each(json.data, function(index, obj) {
                if (obj.DT_RowId === "row_" + supId) {
                    CourseEditModal({course_id: supId, course_name: obj.course_name});
                    return false;
                }
            });
        }, false);
   ...
   // Send course list through email
    $("#email-btn").on( "click", function (e) {
        ...
        var thehtml = dt.html();
        EmailModal({template: "default", subject: "Course List", title:"Course List", html: thehtml});
    });
