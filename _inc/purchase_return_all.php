...
if ($request->server['REQUEST_METHOD'] == 'POST' && $request->post['action_type'] == 'RETURN') {
  try {
    ...
    foreach ($items as $item) {
      ...
      array_push($suppliersAll, array('sup_id' => $sup_id, 'invoice_id' => $invoice_id, 'id' => $item['id'], 'quantity' => $item_quantity));
    }

    // throw new Exception(trans('error_invoice_not_found'));
    //EDGAR 

    $byGroupSupplier = group_by("sup_id", $suppliersAll);
    // if ($checked_item <= 0) {
    ...
    foreach ($byGroupSupplier as $Key => $Sup) {
      // echo $Sup->sm_id;
      $sup_id = $Key; //$item_info['sup-sup_id'];

      $byGroupInvoice = group_by("invoice_id", $Sup);
      foreach ($byGroupInvoice as $Key => $Invoice) {
        $itemsOK = $Invoice;
        ...
        foreach ($itemsOK as $itemOk) {
          ...
          //Edgar New
          $statement = db()->prepare("SELECT `purchase_item`.*, `purchase_info`.`sup_id` FROM `purchase_item` LEFT JOIN `purchase_info` ON (`purchase_info`.`invoice_id` = `purchase_item`.`invoice_id` AND `purchase_info`.`store_id` = `purchase_item`.`store_id`) WHERE `purchase_item`.`id` = ?");
          $statement->execute(array($itemOK_id));
          $item_info = $statement->fetch(PDO::FETCH_ASSOC);
          ...
          $statement = db()->prepare("INSERT INTO `purchase_return_items` SET `store_id` = ?, `invoice_id` = ?, `item_id` = ?, `item_name` = ?, `item_quantity` = ?, `item_price` = ?, `item_tax` = ?, `cgst` = ?, `sgst` = ?, `igst` = ?, `item_total` = ?, `created_at` = ?");
          $statement->execute(array($store_id, $invoice_id, $item_id, $item_name, $item_quantity, $item_price, $item_tax, $cgst, $sgst, $igst, $item_total, date_time()));
        }
    ...
    $Hooks->do_action('After_Purchase_Return', $Invoiceok);

    header('Content-Type: application/json');
    echo json_encode(array('msg' => trans('text_return_success'), 'id' => $Invoiceok));
    exit();
  } catch (Exception $e) {
