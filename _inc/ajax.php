...
if ($request->server['REQUEST_METHOD'] == 'GET' and $request->get['type'] == 'STOCKITEMS') {
	try {
		...
		// $statement = db()->prepare("SELECT `purchase_item`.*, `purchase_info`.`inv_type` FROM `purchase_item` LEFT JOIN `purchase_info` ON (`purchase_item`.`invoice_id` = `purchase_info`.`invoice_id`) WHERE `purchase_item`.`store_id` = ? AND `purchase_item`.`item_quantity` > `purchase_item`.`total_sell` AND `purchase_item`.`status` IN ('stock','active') AND `purchase_info`.`inv_type` = ? ORDER BY `purchase_item`.`item_name`");
		$statement = db()->prepare("SELECT `purchase_item`.*, `purchase_info`.`inv_type` FROM `purchase_item` LEFT JOIN `purchase_info` ON (`purchase_item`.`invoice_id` = `purchase_info`.`invoice_id`) WHERE `purchase_item`.`store_id` = ? AND (`purchase_item`.`item_quantity` - `purchase_item`.`return_quantity`) > `purchase_item`.`total_sell` AND `purchase_item`.`status` IN ('stock','active') AND `purchase_info`.`inv_type` = ? ORDER BY `purchase_item`.`item_name`");
...
if ($request->server['REQUEST_METHOD'] == 'GET' and $request->get['type'] == 'STOCKITEM') {
	try {
		...
		$statement = db()->prepare("SELECT * FROM `purchase_item` WHERE `id` = ? AND (`item_quantity` - `return_quantity`) > `total_sell` AND `status` IN ('stock','active')");
