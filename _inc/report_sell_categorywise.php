...
//   WHERE $where_query
//   GROUP BY selling_item.category_id,CAST(selling_info.created_at AS date)
//   ORDER BY CAST(selling_info.created_at AS date) DESC) as selling_item";

// DB table to use
$table = "(SELECT selling_info.invoice_id, CAST(selling_info.created_at AS date) as created_at, selling_item.id, selling_item.category_id, selling_item.item_name, 
SUM(selling_item.item_quantity -return_quantity) as total_item, SUM(selling_item.item_discount) as discount, 
