...
class ModelPmethod extends Model 
{
	...
	public function updateStatus($pmethod_id, $status, $store_id = null) 
	{
		...
		$statement = $this->db->prepare("UPDATE `pmethods` SET `status` = ? WHERE `pmethod_id` = ?");
		$statement->execute(array((int)$status, (int)$pmethod_id));
	}
	...
	public function getPmethod($pmethod_id, $store_id = null) 
	{
		...
		$statement = $this->db->prepare("SELECT `pmethods`.*
		FROM `pmethods` 
		WHERE `pmethods`.`pmethod_id` = ?");
	...
	public function getPmethods($data = array(), $store_id = null) 
	{
		...
		$sql = "SELECT * FROM `pmethods` WHERE `status` = ?";
