...
class ModelCollege extends Model 
{
	...
	public function updateStatus($college_id, $status, $store_id = null) 
	{
		...
		$statement = $this->db->prepare("UPDATE `colleges` SET `status` = ? WHERE `college_id` = ?");
		$statement->execute(array((int)$status, (int)$college_id));
	}
	...
	public function getColleges($data = array(), $store_id = null) 
	{
		...
		$sql = "SELECT * FROM `colleges` WHERE `status` = ?";
	...
	{
		$store_id = $store_id ? $store_id : store_id();
		$statement = $this->db->prepare("SELECT `p`.`product_id` FROM `product_to_store` as p
		LEFT JOIN `product_to_college` as c2s ON (`p`.`product_id` = `c2s`.`product_id`) 
		WHERE `p`.`store_id` = ? AND `c2s`.`college_id` = ? AND `p`.`status` = ? AND `c2s`.`status` = ?");
		$statement->execute(array($store_id, (int)$college_id, 1, 1));
	...
	public function totalToday($store_id = null) 
	{
		...
		$statement = $this->db->prepare("SELECT * FROM `colleges` WHERE $where_query");
		$statement->execute(array());
	...
	public function getProductStoreCollege($store_id, $college_id)
	{
		// Fetch stores related to products
		$statement = $this->db->prepare("SELECT * FROM `product_to_store` as p
		LEFT JOIN `product_to_college` as c2s ON (`p`.`product_id` = `c2s`.`product_id`) 
		 WHERE `p`.`store_id` = ? and `p`.`status` = 1 and `c2s`.`status` = 1 and `c2s`.college_id = ?  ");
		$statement->execute(array($store_id, $college_id));
