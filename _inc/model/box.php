...
class ModelBox extends Model
{
	...
	public function updateStatus($box_id, $status, $store_id = null)
	{
		...
		$statement = $this->db->prepare("UPDATE `boxes` SET `status` = ? WHERE `box_id` = ?");
		$statement->execute(array((int)$status, (int)$box_id));
	}
	...
	public function getBoxes($data = array(), $store_id = null)
	{
		...
		$sql = "SELECT * FROM `boxes` WHERE `status` = ?";
	...
	public function total($store_id = null)
	{
		...
		$statement = $this->db->prepare("SELECT * FROM `boxes` where `status` = ?");
		$statement->execute(array(1));
