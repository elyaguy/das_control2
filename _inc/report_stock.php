...
// DB table to use
$table = "(SELECT products.p_id, products.p_name, p2s.store_id, p2s.sup_id, p2s.quantity_in_stock, p2s.sell_price as sell_price, p2s.purchase_price as purchase_price , p2c.course_name
FROM products 
LEFT JOIN product_to_store p2s ON (products.p_id = p2s.product_id)
LEFT JOIN courses p2c ON (p2s.course_id = p2c.course_id)
WHERE $where_query
