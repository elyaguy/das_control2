...
// DB table to use
// EYE se agrega totalizado de items
$table = "(SELECT purchase_info.*, purchase_price.payable_amount, purchase_price.paid_amount, purchase_price.due, a.qty FROM `purchase_info` 
  LEFT JOIN `purchase_price` ON (purchase_info.invoice_id = purchase_price.invoice_id) 

  LEFT JOIN (SELECT invoice_id ,SUM(item_quantity) qty FROM `purchase_item` GROUP BY invoice_id ) AS a ON (purchase_info.invoice_id = a.invoice_id)
...
$columns = array(
  ...
  array(
    ...
    'dt' => 'qty',
    'formatter' => function ($d, $row) use ($invoice_model) {
      return currency_format($row['qty']);
    }
  ),
  array(
    'db' => 'due',
